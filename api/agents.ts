...
import { insertAgentSchema } from '../shared/schema.js'; // Adjust to your schema
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (hasId && (isNaN(agentId) || agentId <= 0)) {
  ...
  try {
    ...
    if (!hasId && normalizedUrl === '/api/agents') {
      if (req.method === 'GET') {
        ...
        const agents = await storage.getAgents(); // Adjust to your storage method
        ...
        if (!data) return;
        const result = await storage.createAgent(data); // Adjust to your storage method
    ...
    // Dynamic route: /api/agents/:id
      ...
      if (req.method === 'GET') {
        ...
        const agent = await storage.getAgent(agentId); // Adjust to your storage method
        ...
        const data = validateBody(insertAgentSchema.partial(), req, res); // Adjust schema if needed
        if (!data) return;
        const agent = await storage.updateAgent(agentId, data); // Adjust to your storage method
        if (!agent) {
          return res.status(404).json({ message: 'Agent not found' });
        }
        return res.status(200).json(agent);
        ...
        const success = await storage.deleteAgent(agentId); // Adjust to your storage method
        if (!success) {
          return res.status(404).json({ message: 'Agent not found' });
        }
